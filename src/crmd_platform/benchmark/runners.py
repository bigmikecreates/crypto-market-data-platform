...
class CandlePipelineRunner(PipelineRunner):
    ...
    def run_verbose(
        ...
        open_arr = to_decimal128(open_vals, "open", key)
        high_arr = to_decimal128(high_vals, "high", key)
        low_arr = to_decimal128(low_vals, "low", key)
        close_arr = to_decimal128(close_vals, "close", key)
        volume_arr = to_decimal128(volume_vals, "volume", key)
        ctx.checkpoint("decimal128 cast")

        ts_arr = to_timestamp(ts_vals, ts_config)

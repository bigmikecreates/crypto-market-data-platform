from datetime import datetime, timezone

import pytest

from crmd_platform.models.candle import Candle
from crmd_platform.providers.gemini import (
    GeminiProvider,
    _parse_row,
    _to_gemini_symbol,
    _to_gemini_timeframe,
)
from tests.conftest import load_fixture as _load_fixture


class TestGeminiHelpers:
    def test_to_gemini_symbol_native(self) -> None:
        assert _to_gemini_symbol("BTCUSD") == "BTCUSD"

    def test_to_gemini_symbol_canonical(self) -> None:
        assert _to_gemini_symbol("BTC/USD") == "BTCUSD"

    def test_to_gemini_symbol_eth(self) -> None:
        assert _to_gemini_symbol("ETH/USD") == "ETHUSD"

    def test_to_gemini_symbol_lowercase(self) -> None:
        assert _to_gemini_symbol("btc/usd") == "BTCUSD"

    def test_to_gemini_timeframe_1m(self) -> None:
        assert _to_gemini_timeframe("1m") == "1m"

    def test_to_gemini_timeframe_5m(self) -> None:
        assert _to_gemini_timeframe("5m") == "5m"

    def test_to_gemini_timeframe_1h(self) -> None:
        assert _to_gemini_timeframe("1h") == "1h"

    def test_to_gemini_timeframe_6h(self) -> None:
        assert _to_gemini_timeframe("6h") == "6h"

    def test_to_gemini_timeframe_1d(self) -> None:
        assert _to_gemini_timeframe("1d") == "1d"

    def test_to_gemini_timeframe_invalid(self) -> None:
        with pytest.raises(ValueError, match="Unsupported timeframe"):
            _to_gemini_timeframe("99z")


class TestGeminiProvider:
    def setup_method(self) -> None:
        self.provider = GeminiProvider(rate_limit_sleep=0)
        self.start = datetime(2024, 1, 1, tzinfo=timezone.utc)
        self.end = datetime(2024, 1, 2, tzinfo=timezone.utc)

    def test_provider_is_ohlcv_provider(self) -> None:
        from crmd_platform.providers.base import OHLCVProvider

        assert isinstance(self.provider, OHLCVProvider)

    def test_returns_list_of_candles(self) -> None:
        rows = _load_fixture("gemini_ohlc.json")
        assert isinstance(rows, list)
        assert len(rows) == 5
        for row in rows:
            assert len(row) == 6

    def test_parse_row_returns_candle(self) -> None:
        rows = _load_fixture("gemini_ohlc.json")
        c = _parse_row(rows[0], "gemini", "BTC/USD", "1h", "gemini")
        assert isinstance(c, Candle)
        assert c.exchange == "gemini"
        assert c.symbol == "BTC/USD"
        assert c.timeframe == "1h"
        assert c.timestamp == "2024-01-01T00:00:00"
        assert c.open == "42150.0"
        assert c.high == "42300.0"
        assert c.low == "42100.0"
        assert c.close == "42250.0"
        assert c.volume == "98.5"
        assert c.source == "gemini"

    def test_parse_multiple_rows(self) -> None:
        rows = _load_fixture("gemini_ohlc.json")
        candles = [_parse_row(r, "gemini", "BTC/USD", "1h", "gemini") for r in rows]
        assert len(candles) == 5
        assert candles[0].timestamp == "2024-01-01T00:00:00"
        assert candles[1].timestamp == "2024-01-01T01:00:00"
        assert candles[2].timestamp == "2024-01-01T02:00:00"
        assert candles[3].timestamp == "2024-01-01T03:00:00"
        assert candles[4].timestamp == "2024-01-01T04:00:00"

    def test_exchange_and_source_match(self) -> None:
        rows = _load_fixture("gemini_ohlc.json")
        c = _parse_row(rows[0], "gemini", "BTC/USD", "1h", "gemini")
        assert c.source == c.exchange

    def test_fixture_timestamps_are_sequential(self) -> None:
        rows = _load_fixture("gemini_ohlc.json")
        for i in range(1, len(rows)):
            prev_ts = int(rows[i - 1][0])
            curr_ts = int(rows[i][0])
            assert curr_ts > prev_ts
            assert curr_ts - prev_ts == 3_600_000
